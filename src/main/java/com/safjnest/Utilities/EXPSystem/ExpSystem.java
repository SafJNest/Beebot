...
public class ExpSystem {
    ...
    public int addExp(String userId, String guildId){
        String query = "select exp, level from exp_table where user_id ='"+userId+"' and guild_id = '"+guildId+"';";
        ...
        query = "select exp, level, messages from exp_table where user_id ='"+userId+"' and guild_id = '"+guildId+"';";
        arr = DatabaseHandler.getSql().getSpecifiedRow(query, 0);
        int exp = Integer.valueOf(arr.get(0)) + calculateExp();
