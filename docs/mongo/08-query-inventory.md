...
| contextual ranking | Mongo cursor over `competitive {queue,tier,region}` or `profile_records {filterKey,metric,region}` into a temporary Redis ZSET, then atomic publish | streaming batch | RankingService |
