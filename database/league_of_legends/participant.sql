CREATE TABLE
  `participant` (
    ...
    PRIMARY KEY (`id`),
    KEY `fk_participant_match` (`match_id`),
    KEY `eee` (`summoner_id`),
    CONSTRAINT `eee` FOREIGN KEY (`summoner_id`) REFERENCES `summoner` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_participant_match` FOREIGN KEY (`match_id`) REFERENCES `match` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
