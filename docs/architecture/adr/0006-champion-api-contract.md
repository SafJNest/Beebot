...
Champion statistics, builds and profile statistics use one Kryo configuration for the current model classes. During development, the `com.safjnest.lol.build` package was renamed to `com.safjnest.lol.champion`; old champion build blobs are therefore intentionally invalidated and regenerated by the refresh flow. No compatibility alias or legacy build decoder is maintained for that package migration. If a persisted payload cannot be decoded or validated, the data is treated as missing so the existing refresh flow can recreate it; invalid champion build rows may be deleted during this development migration.
