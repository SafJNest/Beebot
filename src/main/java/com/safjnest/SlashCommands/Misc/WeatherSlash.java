...
public class WeatherSlash extends SlashCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        try {
            InputStream file = new URL("https:" + iconURL).openStream();
            eb.setThumbnail("attachment://icon.png");
            event.deferReply(false).addFiles(FileUpload.fromData(file, "icon.png")).setEmbeds(eb.build()).queue();
        } catch (IOException e) {
            e.printStackTrace();
            event.deferReply(false).addFiles().setEmbeds(eb.build()).queue();
        }
    }
}