...
> Canonical source. Shimmed to `.cursor/rules/`, `.claude/agents/`, `.codex/agents/`, `.opencode/agent/`.
> Read the plan, not the code. Coordinates Builder + Guardian, owns no LoL code.
...
## Example: `docs/proposals/domain-event-match-record.md`

Decomposition (illustrative):

| id | scope | domain | depends_on | parallel_safe | guardian_checks |
|---|---|---|---|---|---|
| U1 | `lol/event/LeagueEventListener.java`, `DomainEvent.java`, `SummonerKey.java` | event | — | true | new file, no ownership conflict |
| U2 | `lol/service/MatchRecordAnalyzer.java` | service | — | true | new file, canonical model only |
| U3 | `lol/model/ProfileStats.java`, `DeltaQuery.java` | model | — | true | `lol.model` only, success DTOs |
| U4 | `lol/service/DeltaQueryBuilder.java` | service | U3 | true | uses `lol.model` only |
| U5 | `MongoDB.bulkUpsertIfBetter`, `execute(DeltaQuery)`, `upsertChampionStatsDelta` | mongo | U3, U4 | false (one file) | `explain IXSCAN` mandatory |
| U6 | `ProfileRecordService implements LeagueEventListener` | service | U1, U2, U5 | true | subscription pattern correct |
| U7 | `ProfileService.onMatchCreated` + `processMatchStats` | service | U3, U4, U5 | true | listener on `MATCH_CREATED` |
| U8 | `MatchService.insert` hook + `SummonerService.refreshProfile` hook | service | U1 | false (one-pass) | publish async, no blocking |
| U9 | `ComputeScheduler.MATCH_INGEST` | queue | U6, U7 | true | per-shard serialization |
| U10 | `LoLBootstrap.register()` | service | U1 | true | force class load |
| U11 | `ProfileRecordServiceTest` | test | U6 | true | `beebot_test` naming |
| U12 | `ProfileStatsTest`, `DeltaQueryBuilderTest` | test | U3, U4, U5 | true | unit tests no Mongo |
| U13 | docs (HANDBOOK, architecture, mongo) + CHANGELOG | doc | U5-U9 | true | sync with code changes |

Schedule:
- Wave 1: U1, U2, U3 in parallel.
- Wave 2: U4, U5, U8, U10 in parallel (U5 waits U3+U4 internally; orchestrator just ensures they don't conflict on files).
- Wave 3: U6, U7, U9, U11, U12, U13 in parallel.
- Wave 4: final guardian sweep.
